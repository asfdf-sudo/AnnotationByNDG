...
public abstract class JmsUtils {
	 ...
	 */
	public static void commitIfNecessary( @Nullable Session session) throws JMSException {
	 ...
	 * @throws JMSException if committing failed
	 */
	public static void rollbackIfNecessary( @Nullable Session session) throws JMSException {
		Assert.notNull(session, "Session must not be null");
		try {
			session.rollback();
		}
