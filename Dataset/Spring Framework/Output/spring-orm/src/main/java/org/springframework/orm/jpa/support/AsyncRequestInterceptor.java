...
class AsyncRequestInterceptor implements CallableProcessingInterceptor, DeferredResultProcessingInterceptor {
	...
	public void bindEntityManager() {
		...
		public <T> Object handleTimeout( @Nullable NativeWebRequest request,  @Nullable Callable<T> task) {
		this.timeoutInProgress = true;
		return RESULT_NONE;  // give other interceptors a chance to handle the timeout
	}

		public <T> Object handleError( @Nullable NativeWebRequest request,  @Nullable Callable<T> task,  @Nullable Throwable t) {
		this.errorInProgress = true;
		return RESULT_NONE;  // give other interceptors a chance to handle the error
	}

		public <T> void afterCompletion( @Nullable NativeWebRequest request,  @Nullable Callable<T> task) throws Exception {
		closeEntityManager();
	}
	...
	// Implementation of DeferredResultProcessingInterceptor methods
		...
		public <T> boolean handleTimeout( @Nullable NativeWebRequest request,  @Nullable DeferredResult<T> deferredResult) {
		this.timeoutInProgress = true;
