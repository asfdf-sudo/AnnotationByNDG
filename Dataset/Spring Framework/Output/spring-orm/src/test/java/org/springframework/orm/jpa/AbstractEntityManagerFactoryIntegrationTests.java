...
public abstract class AbstractEntityManagerFactoryIntegrationTests {
	...
	protected int countRowsInTable(EntityManager em,  @Nullable String tableName) {
		Query query = em.createNativeQuery("SELECT COUNT(0) FROM " + tableName);
		return ((Number) query.getSingleResult()).intValue();
	}

	protected int countRowsInTable( @Nullable String tableName) {
		return this.jdbcTemplate.queryForObject("SELECT COUNT(0) FROM " + tableName, Integer.class);
	}

	protected void executeSqlScript( @Nullable String sqlResourcePath) throws DataAccessException {
