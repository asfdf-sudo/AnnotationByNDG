...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, Class<?> targetClass,
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager cpptm)) {
			...
			try {
				result = cpptm.<Object> execute(txAttr, status -> {
	 ...
	 */
		 @Nullable protected TransactionManager determineTransactionManager(TransactionAttribute txAttr) {
		return null;
	}
	 ...
	 */
	 @Nullable protected String methodIdentification(Method method, Class<?> targetClass) {
		return null;
	}
	...
	private class ReactiveTransactionSupport {
		...
		private Mono<Void> completeTransactionAfterThrowing(
				ReactiveTransactionInfo txInfo, InvocationCallback invocation, Throwable ex) {
