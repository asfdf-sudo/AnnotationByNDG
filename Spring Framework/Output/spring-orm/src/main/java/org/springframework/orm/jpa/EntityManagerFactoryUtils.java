...
public abstract class EntityManagerFactoryUtils {
	 ...
	 */
	 @Nullable public static EntityManager doGetTransactionalEntityManager(
			 @Nullable EntityManagerFactory emf,  @Nullable Map<?, ?> properties, boolean synchronizedWithTransaction)
			throws PersistenceException {
	 ...
	 */
	private static void cleanupTransaction( @Nullable Object transactionData,  @Nullable EntityManagerFactory emf) {
		if (emf instanceof EntityManagerFactoryInfo emfInfo) {
			JpaDialect jpaDialect = emfInfo.getJpaDialect();
			if (jpaDialect != null) {
