...
class PersistenceManagedTypesBeanRegistrationAotProcessor implements BeanRegistrationAotProcessor {
	...
	private static final class JpaManagedTypesBeanRegistrationCodeFragments extends BeanRegistrationCodeFragmentsDecorator {
		...
		private void contributeHints(RuntimeHints hints, ClassLoader classLoader, List<String> managedClassNames) {
		...
		private void contributeHibernateHints(RuntimeHints hints, ClassLoader classLoader, Class<?> managedClass) {
			ReflectionHints reflection = hints.reflection();
		...
		private void contributePackagePrivateHints(RuntimeHints hints, Class<?> managedClass) {
			...
			ReflectionUtils.doWithMethods(managedClass, method ->
				...
				private static Class<? extends Annotation> loadClass(String className, ClassLoader classLoader) {
			try {
		...
		// Not-null assertion performed in ReflectionHints.registerType
		private void registerForReflection(ReflectionHints reflection, Annotation annotation, String attribute) {
			if (annotation == null) {
				return;
			}
